...
const path = require('path'); // 경로 관련 모듈 사용
...
targetFolderList.forEach((folder) => {
  ...
  if (fs.existsSync(folderPath) && fs.statSync(folderPath).isDirectory()) {
    generateIndexFile(folderPath); // 대상 폴더에서 index.ts 파일 생성 함수 호출
  }
});

let count = 0;

// 지정된 디렉토리 내의 파일들을 기반으로 index.ts 파일을 생성하는 함수
function generateIndexFile(directoryPath) {
  // 현재 디렉토리 내의 index.ts 파일 경로 생성
  ...
  const files = fs.readdirSync(directoryPath); // 디렉토리 내의 파일 목록을 읽어옴
  ...
  files.forEach((file) => {
    ...
    if (fs.statSync(filePath).isFile() && (file.endsWith('.ts') || file.endsWith('.tsx')) && file !== 'index.ts') {
      // 파일이 존재하고 확장자가 .ts 또는 .tsx이며, 파일 이름이 index.ts가 아닌 경우
      exportStatementList.push(`export * from './${fileNameWithoutExtension}';`);
    } else if (fs.statSync(filePath).isDirectory() && file !== 'index.ts') {
      // 디렉토리인 경우, 파일 이름이 index.ts가 아닌 경우
      exportStatementList.push(`export * from './${fileNameWithoutExtension}';`);
      generateIndexFile(filePath); // 재귀적으로 하위 디렉토리 처리
    }
  });

  const content = exportStatementList.join('\n'); // export 구문을 줄 바꿈으로 연결하여 내용 생성
  fs.writeFileSync(indexFilePath, content); // 생성된 내용을 index.ts 파일에 쓰기

  console.log(`${++count} ${`\t`} ${relativePath}`); // 생성된 index.ts 파일 경로를 로그로 출력
}
